...
class Config:
    ...
    # Cloudflare Worker (IP rotation middleware)
    CLOUDFLARE_WORKER_URL = os.getenv("CLOUDFLARE_WORKER_URL", "").rstrip("/")
    WORKER_API_KEY = os.getenv("WORKER_API_KEY", "")
    USE_WORKER = bool(CLOUDFLARE_WORKER_URL)
...
class WorkerClient:
    """
    Gọi SmailPro qua Cloudflare Worker.
    IP thay đổi mỗi request → không bao giờ bị 429.
    """
    ...
    def get_inbox(self, email: TempEmail) -> tuple[Optional[str], List[InboxMessage]]:
        ...
        # Get messages từ Sonjj (cũng qua Worker)
        messages = self._fetch_sonjj_inbox(payload)
        return payload, messages
...
# Chọn client dựa trên environment
if Config.USE_WORKER:
    print("🌐 Using Cloudflare Worker for IP rotation")
    proxy_rotator = None
    client = WorkerClient()
else:
...
def api_health():
    ...
    return jsonify({
        ...
        "worker_url": Config.CLOUDFLARE_WORKER_URL if Config.USE_WORKER else None,
        "proxies": len(proxy_rotator.proxies) if proxy_rotator else 0
    })
...
if __name__ == "__main__":
    ...
    print(f"🌐 Mode: {'Worker (IP Rotation)' if Config.USE_WORKER else 'Direct Connection'}")
    if Config.USE_WORKER:
        print(f"🔗 Worker URL: {Config.CLOUDFLARE_WORKER_URL}")
    else:
