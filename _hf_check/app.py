    ...
    try:
        if "callback_query" in update:
            ...
            if text.startswith("/start"):
                ...
                logger.info(f"/start from {uid}, calling send_menu...")
                try:
                    send_menu(chat, uid)
                    logger.info(f"/start send_menu OK for {uid}")
                except Exception as menu_err:
                    logger.error(f"/start send_menu FAILED for {uid}: {menu_err}", exc_info=True)
                    # Fallback: send plain text
                    tg_api("sendMessage", {"chat_id": chat, "text": "👋 Chào mừng! Bot đang gặp lỗi, thử lại sau."})
...
def send_menu(chat, uid, mid=None):
    ...
    if mid:
        result = tg_api("editMessageText", {"chat_id": chat, "message_id": mid, "text": WELCOME_MESSAGE, "parse_mode": "HTML", "reply_markup": k})
    else:
        result = tg_api("sendMessage", {"chat_id": chat, "text": WELCOME_MESSAGE, "parse_mode": "HTML", "reply_markup": k})
    logger.info(f"send_menu chat={chat} uid={uid} mid={mid} result={result}")
    return result
...
def do_task_game(chat, uid, mid):
    ...
    tg_api("editMessageText", {"chat_id": chat, "message_id": mid,
        ...
        "reply_markup": kb([[btn("🔄 Quay lại", callback_data="task_game")], [btn("📧 Tạo Email", callback_data="create_email")], [btn("🏠 Menu", callback_data="main_menu")]])})
...
def debug():
    """Debug endpoint — test bot connectivity"""
    results = {}
    
    # 1. Check env vars
    results["bot_token_set"] = bool(BOT_TOKEN)
    results["worker_url_set"] = bool(WORKER_URL)
    results["worker_key_set"] = bool(WORKER_KEY)
    results["webhook_url"] = WEBHOOK_URL or "(not set)"
    
    # 2. Test Worker proxy
    if WORKER_URL:
        try:
            resp = requests.get(f"{WORKER_URL}/health", headers={"X-API-Key": WORKER_KEY}, timeout=10)
            results["worker_health"] = {"status": resp.status_code, "body": resp.text[:200]}
        except Exception as e:
            results["worker_health"] = {"error": str(e)}
    ...
    try:
        ...
        results["tg_getMe"] = {"error": str(e)}
    
    # 4. Test webhook info
    try:
        wh = tg_api("getWebhookInfo", timeout=15)
        results["webhook_info"] = wh
    except Exception as e:
        results["webhook_info"] = {"error": str(e)}
